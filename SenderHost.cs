...
public class SenderHost:BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        try
        {
            ...
            var broker=EventBrokerFactory.GetEventBroker(brokerConfig,_logger);

            var i = 0;
            _logger.Verbose("Sender started");
            
            Console.WriteLine("Define number of Events/minute :");
            var frequencyPerSec=Convert.ToInt32(Console.ReadLine());
            ...
            while (true)
            {
                ...
                await Task.Delay((60*1000)/frequencyPerSec,stoppingToken);   
            }
            return ;
        }
        catch (Exception e)
